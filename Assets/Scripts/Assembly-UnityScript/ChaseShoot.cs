...
public class ChaseShoot : MonoBehaviour
{
	...
	public virtual void Attack()
	{
		...
		GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate(projectile, thisTransform.position + vector, transform.rotation);
		gameObject.transform.LookAt(targetObject.transform);
		Vector3 vector2 = targetObject.transform.position - gameObject.transform.position;
		vector2.Normalize();
		gameObject.GetComponent<Rigidbody>().velocity = vector2 * projectileSpeed;
		AudioSource.PlayClipAtPoint(shootSound, thisTransform.position + vector);
		gameObject.SendMessage("SetDamage", attackDamage);
	}
